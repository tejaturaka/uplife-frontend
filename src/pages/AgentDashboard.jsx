...
import { fetchLocations, getMandals } from '../services/locationService';
import Logo from '../components/Logo';
import { UserPlus, LogOut, MapPin, X, ShieldCheck, ArrowUpCircle, XCircle, Users, Activity, Award, FileText } from 'lucide-react';
import { useNavigate } from 'react-router-dom';
import { motion, AnimatePresence } from 'framer-motion';
import axios from 'axios';
...
export default function AgentDashboard() {
  ...
  const [form, setForm] = useState({ name: '', email: '', state: '', dist: '', mandal: '', password: '' });
  ...
  const fetchData = async () => {
    try {
      const response = await axios.get(`${API_BASE}/api/users/all`);
      // Agents only see users created by them OR in their district (Simplified here to all users for now or filter by logic)
      // Since specific logic wasn't provided, I will show all users but typically you might filter: u.createdBy === session.id
      setUsers(response.data.filter(u => u.role === 'user'));
    } catch (e) { console.error("Sync Error"); }
  };

  const handleOpenModal = () => {
    setForm({ name: '', email: '', state: '', dist: '', mandal: '', password: '' });
    setShowModal(true);
  };
  ...
  const handleFormSubmit = async (e) => {
    e.preventDefault();
    try {
        const payload = { ...form, role: 'user', claimStatus: 0, statusMessage: "PENDING", createdBy: session?.id || "AGENT" };
        const res = await axios.post(`${API_BASE}/api/users/register`, payload);
        alert(`SUCCESS! Generated User ID: ${res.data.id}`);
        setShowModal(false); 
        fetchData();
    } catch(err) { alert("Registration Failed."); }
  };
    ...
    <div className="min-h-screen bg-[#f8fafc] flex text-left font-sans">
      <aside className="w-[340px] bg-white border-r p-8 fixed h-full flex flex-col z-20 shadow-sm">
        ...
        <div className="bg-teal-50 p-6 rounded-3xl mb-8">
            <p className="text-[10px] font-black uppercase text-teal-600 mb-1">Agent ID</p>
            <p className="text-xl font-black text-slate-900">{session?.id || "Unknown"}</p>
        </div>
        <nav className="space-y-3 flex-1">
          <button className="w-full p-5 rounded-3xl flex items-center gap-4 font-black text-xs uppercase bg-slate-900 text-white shadow-2xl"><Activity size={20}/> My Customers</button>
          <button onClick={handleOpenModal} className="w-full p-5 rounded-3xl flex items-center gap-4 font-black text-xs uppercase text-indigo-600 bg-indigo-50/50 border border-indigo-100 hover:bg-indigo-50 transition-all"><UserPlus size={20}/> Register New</button>
        </nav>
        <button onClick={() => { localStorage.removeItem('session'); navigate('/'); }} className="flex items-center gap-4 text-red-500 font-black p-6 hover:bg-red-50 transition-all"><LogOut size={22}/> Exit System</button>
      </aside>
      ...
      <main className="ml-[340px] flex-1 p-16">
        <header className="flex justify-between items-center mb-16 relative">
          <h1 className="text-6xl font-black text-slate-900 tracking-tighter">Agent <span className="text-[#00ced1]">Portal</span></h1>
        </header>
        ...
        <div className="bg-white rounded-[4rem] shadow-2xl p-12 border border-white">
            ...
            <div className="grid gap-4">
                {users.map(item => {
                    ...
                    <div key={item.id} className="flex justify-between items-center p-6 bg-[#fcfdfe] rounded-[2.5rem] border border-slate-50 hover:border-[#00ced1] transition-all">
                        <div className="flex items-center gap-6 w-1/2">
                            <div className="w-14 h-14 rounded-2xl flex items-center justify-center font-black text-xl shrink-0 bg-indigo-50 text-indigo-600">
                                {item.name ? item.name.charAt(0).toUpperCase() : 'U'}
                            </div>
                            <div className="text-left">
                                <p className="font-black text-2xl text-slate-900">{item.name}</p>
      ...
      <AnimatePresence>
        {showModal && (
          <div className="fixed inset-0 z-[100] flex items-center justify-center p-6 bg-slate-900/60 backdrop-blur-md">
            <motion.div initial={{ scale: 0.9, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} className="bg-white w-full max-w-lg rounded-[4rem] p-12 shadow-3xl border border-white text-left relative overflow-hidden">
                <button onClick={() => setShowModal(false)} className="absolute top-10 right-10 p-3 bg-slate-50 hover:bg-red-50 hover:text-red-500 rounded-2xl transition-all cursor-pointer text-slate-400"><X size={24}/></button>
                <h2 className="text-4xl font-black text-slate-900 mb-2 tracking-tighter">Register <span className="text-indigo-600">Customer</span></h2>
                <form onSubmit={handleFormSubmit} className="space-y-4 mt-6">
                  ...
                  <select disabled={!form.dist} className="w-full p-5 bg-slate-50 rounded-2xl font-bold outline-none" value={form.mandal} onChange={e => setForm({...form, mandal: e.target.value})} required><option value="">Select Mandal</option>{mandals.map(m => <option key={m} value={m}>{m}</option>)}</select>
                  <input type="password" placeholder="Password" required className="w-full p-5 bg-slate-50 rounded-2xl font-bold border-2 border-transparent focus:border-[#00ced1] outline-none" value={form.password} onChange={e => setForm({...form, password: e.target.value})} />
                  <button type="submit" className="w-full py-7 rounded-[2.5rem] font-black text-lg shadow-2xl transition-all mt-6 cursor-pointer bg-indigo-600 text-white">Generate Customer ID</button>
                </form>
            </motion.div>
          </div>
        )}
                ...
                <motion.div initial={{ scale: 0.9, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} exit={{ scale: 0.9, opacity: 0 }} className="bg-white w-full max-w-4xl max-h-[85vh] overflow-y-auto rounded-[3rem] p-12 shadow-3xl border border-white relative">
                    ...
                    {userDocs.length === 0 ? (
                        <div className="p-10 text-center bg-slate-50 rounded-3xl border-2 border-dashed border-slate-200">
                            <p className="font-bold text-slate-400">No documents uploaded.</p>
                        </div>
                    ) : (
                            ...
                            {userDocs.map(doc => (
                                <div key={doc.id} className="bg-slate-50 p-4 rounded-3xl border border-slate-100">
                                    <div className="bg-white rounded-2xl h-48 flex items-center justify-center overflow-hidden border border-slate-100 mb-4">
                                        <img src={`${API_BASE}/api/documents/download/${doc.id}`} alt="Doc" className="w-full h-full object-cover"/>
                                    </div>
