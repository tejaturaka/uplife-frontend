...
// //   const updateStatus = async (user, nextStatus, msg) => {
// //     if(!window.confirm(`Update ${user.name} to: ${msg}?`)) return;
// //     try {
// //       await axios.post(`${API_BASE}/api/users/register`, { ...user, claimStatus: nextStatus, statusMessage: msg });
// //       fetchData(); 
...
// //       <aside className="w-[340px] bg-white border-r p-8 fixed h-full flex flex-col z-20">
// //         <div className="mb-12"><Logo /></div>
// //         <nav className="space-y-3 flex-1">
// //           <button onClick={() => setFilterType('all')} className={`w-full p-5 rounded-3xl flex items-center gap-4 font-black text-xs uppercase tracking-widest ${filterType === 'all' ? 'bg-slate-900 text-white shadow-2xl' : 'text-slate-500 hover:bg-slate-50'}`}><Activity size={20}/> Dashboard Home</button>
// //           <div className="h-px bg-slate-100 my-6 mx-4"></div>
// //           <button onClick={() => { setForm({...form, type:'agent'}); setShowModal(true); }} className="w-full p-5 rounded-3xl flex items-center gap-4 font-black text-xs uppercase text-[#00ced1] bg-teal-50/50 border border-teal-100"><Briefcase size={20}/> Register Agent</button>
// //           <button onClick={() => { setForm({...form, type:'user'}); setShowModal(true); }} className="w-full p-5 rounded-3xl flex items-center gap-4 font-black text-xs uppercase text-indigo-600 bg-indigo-50/50 border border-indigo-100"><Users size={20}/> Register Customer</button>
// //         </nav>
// //         <button onClick={() => { localStorage.removeItem('session'); navigate('/'); }} className="flex items-center gap-4 text-red-500 font-black p-6 hover:bg-red-50 rounded-[2rem] transition-all"><LogOut size={22}/> Exit System</button>
// //       </aside>

// //       <main className="ml-[340px] flex-1 p-16">
// //         <header className="flex justify-between items-center mb-16 relative">
// //           <div><h1 className="text-6xl font-black text-slate-900 tracking-tighter leading-none">Admin <span className="text-[#00ced1]">Dashboard</span></h1><p className="text-slate-400 font-bold mt-3 uppercase tracking-[0.3em] text-[10px]">Secure Node Ledger Control</p></div>
// //           <div className="flex items-center gap-5 bg-white p-3 pr-8 rounded-[2.5rem] shadow-xl border border-white"><div className="w-14 h-14 bg-slate-900 rounded-2xl flex items-center justify-center text-[#00ced1] shadow-lg"><ShieldCheck size={28}/></div><div><p className="text-[9px] font-black uppercase text-[#00ced1] tracking-widest">Primary Administrator</p><p className="text-xl font-black text-slate-900">System Master</p></div></div>
// //         </header>

// //         {/* ORDER: Agents, Customers, Pending, Settled, Rejected */}
// //         <div className="grid grid-cols-5 gap-6 mb-16">
// //           {[
// //             { label: 'Agents', val: stats.agents, icon: <Briefcase size={20}/>, color: 'text-blue-600', bg: 'bg-blue-50' },
// //             { label: 'Customers', val: stats.customers, icon: <Users size={20}/>, color: 'text-indigo-600', bg: 'bg-indigo-50' },
// //             { label: 'Pending', val: stats.pending, icon: <Clock size={20}/>, color: 'text-[#00ced1]', bg: 'bg-teal-50' },
// //             { label: 'Settled', val: stats.settled, icon: <Award size={20}/>, color: 'text-green-600', bg: 'bg-green-50' },
// //             { label: 'Rejected', val: stats.rejected, icon: <XCircle size={20}/>, color: 'text-red-500', bg: 'bg-red-50' },
// //           ].map((stat, i) => (
// //             <div key={i} className="p-8 rounded-[3rem] border-2 border-transparent bg-white shadow-sm hover:shadow-xl transition-all">
// //               <div className={`w-12 h-12 ${stat.bg} ${stat.color} rounded-2xl flex items-center justify-center mb-6`}>{stat.icon}</div>
// //               <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-1">{stat.label}</p>
// //               <p className={`text-4xl font-black ${stat.color} tracking-tighter`}>{stat.val}</p>
// //             </div>
...
// //                 <h3 className="font-black text-3xl capitalize text-slate-900 mb-10">Customer Directory</h3>
// //                 <div className="grid gap-4">
// //                   {db.users.map(item => (
// //                     <div key={item.id} className="flex justify-between items-center p-8 bg-[#fcfdfe] rounded-[2.5rem] border border-slate-50 hover:border-[#00ced1] transition-all">
// //                       <div className="flex items-center gap-6"><div className="w-14 h-14 rounded-2xl flex items-center justify-center font-black text-xl bg-indigo-50 text-indigo-600">{item.name.charAt(0)}</div><div className="text-left"><p className="font-black text-2xl text-slate-900">{item.name}</p><p className="text-[10px] font-black text-slate-400 uppercase tracking-widest mt-1"><MapPin size={12}/> {item.mandal}, {item.dist}</p></div></div>
// //                       <div className="flex items-center gap-4">
// //                         {item.claimStatus !== -1 && (
// //                           <div className="flex gap-2 mr-6 bg-white p-2 rounded-2xl border border-slate-50">
// //                             <button onClick={() => updateStatus(item, item.claimStatus + 1, item.claimStatus + 1 === 3 ? "SETTLED" : `STAGE ${item.claimStatus + 1} DONE`)} className="p-3 bg-[#00ced1] text-white rounded-xl hover:scale-110"><ArrowUpCircle size={22}/></button>
// //                             <button onClick={() => updateStatus(item, -1, "REJECTED")} className="p-3 bg-red-500 text-white rounded-xl hover:scale-110"><XCircle size={22}/></button>
// //                           </div>
// //                         )}
// //                         <div className="text-right px-6 border-r border-slate-100 mr-2"><p className="font-mono text-sm font-black text-slate-800">{item.id}</p><p className={`text-[9px] font-black uppercase tracking-widest mt-1 ${item.claimStatus === -1 ? 'text-red-500' : item.claimStatus === 3 ? 'text-green-600' : 'text-[#00ced1]'}`}>{item.claimStatus === -1 ? 'REJECTED' : (item.claimStatus === 3 ? 'SETTLED' : 'PENDING')}</p></div>
// //                         <div className="flex gap-3"><button onClick={() => startEdit(item)} className="p-4 bg-white text-slate-400 rounded-2xl border hover:text-[#00ced1]"><Edit3 size={20}/></button><button onClick={async () => { if(window.confirm("Delete?")) { await axios.delete(`${API_BASE}/api/users/${item.id}`); fetchData(); }}} className="p-4 bg-white text-slate-400 rounded-2xl border hover:text-red-500"><Trash2 size={20}/></button></div>
// //                       </div>
// //                     </div>
// //                   ))}
// //                 </div>
...
// import Logo from '../components/Logo';
// import { 
//   UserPlus, LogOut, MapPin, Trash2, Edit3, X, ShieldCheck, 
//   ArrowUpCircle, XCircle, Users, Briefcase, Clock, Activity, Award, ChevronRight
// } from 'lucide-react';
// import { useNavigate } from 'react-router-dom';
// import { motion, AnimatePresence } from 'framer-motion';
// import axios from 'axios';
...
//   // --- BUTTON HANDLER: OPEN REGISTRATION MODAL ---
//   const handleOpenModal = (type) => {
...
//     const allItems = [...db.agents, ...db.users];
//     if (filterType === 'all') return allItems;
//     if (filterType === 'agent') return db.agents;
//     if (filterType === 'user') return db.users;
//     if (filterType === 'pending') return db.users.filter(u => u.claimStatus >= 0 && u.claimStatus < 3);
//     if (filterType === 'settled') return db.users.filter(u => u.claimStatus === 3);
//     if (filterType === 'rejected') return db.users.filter(u => u.claimStatus === -1);
//     return allItems;
//   };

//   const updateStatus = async (user, nextStatus, msg) => {
//     if(!window.confirm(`Confirm update for ${user.name} to: ${msg}?`)) return;
//     try {
//       await axios.post(`${API_BASE}/api/users/register`, { ...user, claimStatus: nextStatus, statusMessage: msg });
//       fetchData(); 
//     } catch (error) { alert("Status Update Failed."); }
...
//           {/* FUNCTIONAL BUTTONS */}
//           <button onClick={() => handleOpenModal('agent')} className="w-full p-5 rounded-3xl flex items-center gap-4 font-black text-xs uppercase text-[#00ced1] bg-teal-50/50 border border-teal-100 hover:bg-teal-50 mt-6">
//             <Briefcase size={20}/> Register Agent
//           </button>
//           <button onClick={() => handleOpenModal('user')} className="w-full p-5 rounded-3xl flex items-center gap-4 font-black text-xs uppercase text-indigo-600 bg-indigo-50/50 border border-indigo-100 hover:bg-indigo-50">
//             <Users size={20}/> Register Customer
//           </button>
//         </nav>
...
//         <header className="flex justify-between items-center mb-16 relative">
//           <div><h1 className="text-6xl font-black text-slate-900 tracking-tighter leading-none">Admin <span className="text-[#00ced1]">Dashboard</span></h1><p className="text-slate-400 font-bold mt-3 uppercase tracking-[0.3em] text-[10px]">Secure Node Ledger Control</p></div>
//           <div className="flex items-center gap-5 bg-white p-3 pr-8 rounded-[2.5rem] shadow-xl border border-white"><div className="w-14 h-14 bg-slate-900 rounded-2xl flex items-center justify-center text-[#00ced1] shadow-lg"><ShieldCheck size={28}/></div><div><p className="text-[9px] font-black uppercase text-[#00ced1] tracking-widest mb-0.5">Primary Administrator</p><p className="text-xl font-black text-slate-900 leading-none">System Master</p></div></div>
//         </header>

//         {/* ORDERED & FUNCTIONAL STATS: Agents, Customers, Pending, Settled, Rejected */}
//         <div className="grid grid-cols-5 gap-6 mb-16">
//           {[
//             { label: 'Agents', val: stats.agents, type: 'agent', icon: <Briefcase size={20}/>, color: 'text-blue-600', bg: 'bg-blue-50' },
//             { label: 'Customers', val: stats.customers, type: 'user', icon: <Users size={20}/>, color: 'text-indigo-600', bg: 'bg-indigo-50' },
...
//         {/* DIRECTORY SECTION */}
//         <div className="bg-white rounded-[4rem] shadow-2xl p-12 border border-white">
//             <h3 className="font-black text-3xl capitalize text-slate-900 mb-10">{filterType === 'all' ? 'Customer & Agent' : filterType} Directory</h3>
//             <div className="grid gap-4">
//                 {filteredItems.map(item => (
//                     <div key={item.id} className="flex justify-between items-center p-8 bg-[#fcfdfe] rounded-[2.5rem] border border-slate-50 hover:border-[#00ced1] transition-all">
//                         <div className="flex items-center gap-6">
//                             <div className={`w-14 h-14 rounded-2xl flex items-center justify-center font-black text-xl shadow-inner ${item.role === 'agent' ? 'bg-teal-50 text-[#00ced1]' : 'bg-indigo-50 text-indigo-600'}`}>{item.name.charAt(0).toUpperCase()}</div>
//                             <div className="text-left"><p className="font-black text-2xl text-slate-900">{item.name}</p><p className="text-[10px] font-black text-slate-400 uppercase"><MapPin size={12}/> {item.mandal}, {item.dist}</p></div>
//                         </div>
//                         <div className="flex items-center gap-4">
//                            {item.role === 'user' && item.claimStatus !== -1 && (
//                             <div className="flex gap-2 mr-6 bg-white p-2 rounded-2xl border border-slate-50">
//                                 <button onClick={() => updateStatus(item, item.claimStatus + 1, item.claimStatus + 1 === 3 ? "SETTLED" : `STAGE ${item.claimStatus + 1} DONE`)} className="p-3 bg-[#00ced1] text-white rounded-xl hover:scale-110 cursor-pointer"><ArrowUpCircle size={22}/></button>
//                                 <button onClick={() => updateStatus(item, -1, "REJECTED")} className="p-3 bg-red-500 text-white rounded-xl hover:scale-110 cursor-pointer"><XCircle size={22}/></button>
//                             </div>
//                            )}
//                            <div className="text-right px-6 border-r border-slate-100 mr-2"><p className="font-mono text-sm font-black text-slate-800">{item.id}</p><p className={`text-[9px] font-black uppercase tracking-widest mt-1 ${item.claimStatus === -1 ? 'text-red-500' : item.claimStatus === 3 ? 'text-green-600' : 'text-[#00ced1]'}`}>{item.claimStatus === -1 ? 'REJECTED' : (item.claimStatus === 3 ? 'SETTLED' : 'PENDING')}</p></div>
//                            <div className="flex gap-3">
//                               <button onClick={() => startEdit(item)} className="p-4 bg-white text-slate-400 rounded-2xl border hover:text-[#00ced1] transition-all"><Edit3 size={20}/></button>
//                               <button onClick={async () => { if(window.confirm("Delete record?")) { await axios.delete(`${API_BASE}/api/users/${item.id}`); fetchData(); }}} className="p-4 bg-white text-red-500 rounded-2xl border hover:shadow-lg transition-all"><Trash2 size={20}/></button>
//                            </div>
//                         </div>
//                     </div>
//                 ))}
      ...
      <main className="ml-[340px] flex-1 p-16">
        ...
        <div className="bg-white rounded-[4rem] shadow-2xl p-12 border border-white">
            ...
            <div className="grid gap-4">
                {filteredItems.map(item => (
                    <div key={item.id} className="flex justify-between items-center p-8 bg-[#fcfdfe] rounded-[2.5rem] border border-slate-50 hover:border-[#00ced1] transition-all">
                        <div className="flex items-center gap-6"><div className={`w-14 h-14 rounded-2xl flex items-center justify-center font-black text-xl ${item.role === 'agent' ? 'bg-teal-50 text-[#00ced1]' : 'bg-indigo-50 text-indigo-600'}`}>{item.name.charAt(0).toUpperCase()}</div><div className="text-left"><p className="font-black text-2xl text-slate-900">{item.name}</p><p className="text-[10px] font-black text-slate-400 uppercase"><MapPin size={12}/> {item.mandal}, {item.dist}</p></div></div>
                        <div className="flex items-center gap-4">
                            {item.role === 'user' && item.claimStatus !== -1 && (
                                <div className="flex gap-2 mr-6 bg-white p-2 rounded-2xl border border-slate-50">
                            ...
                            <div className="text-right px-6 border-r border-slate-100 mr-2"><p className="font-mono text-sm font-black text-slate-800">{item.id}</p><p className={`text-[9px] font-black uppercase tracking-widest ${item.claimStatus === -1 ? 'text-red-500' : item.claimStatus === 3 ? 'text-green-600' : 'text-[#00ced1]'}`}>{item.claimStatus === -1 ? 'REJECTED' : (item.claimStatus === 3 ? 'SETTLED' : 'PENDING')}</p></div>
                            <div className="flex gap-3"><button onClick={() => startEdit(item)} className="p-4 bg-white text-slate-400 rounded-2xl border hover:text-[#00ced1] transition-all"><Edit3 size={20}/></button><button onClick={async () => { if(window.confirm("Delete?")) { await axios.delete(`${API_BASE}/api/users/${item.id}`); fetchData(); }}} className="p-4 bg-white text-slate-400 rounded-2xl border hover:text-red-500 transition-all"><Trash2 size={20}/></button></div>
                        </div>
                    </div>
                ))}
            </div>
          ...
          <div className="fixed inset-0 z-[100] flex items-center justify-center p-6 bg-slate-900/60 backdrop-blur-md">
            <motion.div initial={{ scale: 0.9, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} exit={{ scale: 0.9, opacity: 0 }} className="bg-white w-full max-w-lg rounded-[4rem] p-12 shadow-3xl border border-white text-left relative overflow-hidden">
                ...
                <form onSubmit={async (e) => {
                  e.preventDefault();
                  await axios.post(`${API_BASE}/api/users/register`, { ...form, id: editingId || "", role: form.type, claimStatus: editingId ? undefined : 0, statusMessage: editingId ? undefined : "PENDING", createdBy: "ADMIN_MASTER" });
                  alert("Registry Sync Successful."); cancelEdit(); fetchData();
                }} className="space-y-4">
                  ...
                  <button className={`w-full py-7 rounded-[2.5rem] font-black text-lg shadow-2xl transition-all mt-6 ${form.type === 'agent' ? 'bg-[#00ced1] text-white' : 'bg-indigo-600 text-white'}`}>{editingId ? "Update Record" : "Register Node"}</button>
                </form>
            </motion.div>
          </div>
        )}
